...
c      include 'paralelo.copy'
      ...
      if(nref.lt.3 .and. rank .eq. 0)write(6,*)'error interp2c: the reference table only has', nref, ' entries' 
      if(xabs(1).lt.xtab(1) .or. xabs(ntab).gt.xtab(1+(nref-1)*incx)) then
       if (rank .eq. 0) write(6,*)'interp2c: warning: some values lie out of interpolating range'  
      end if
