...
cPL	  This case is called when recalc_total=.true.
     ...
     ;  , status='unknown')
	    ...
	    if(onlyab_now)then
	    if (rank .eq. 0)write(80,*)'active power only'
	    if (rank .eq. 0)write(82,*)'active power only'
	    else
	    if (rank .eq. 0)write(80,*)'active and reactive powers'
	    if (rank .eq. 0)write(82,*)'active and reactive powers'
	    end if
...
cPL     ;  , file = paplda // 'Element quadratic forms_' // paname(icurspe)
     ...
     ;  , status='unknown')
      ...
      if (rank .eq. 0) write(6,1000) sysquf
      if (rank .eq. 0) write(80,1000) sysquf
 1000 format('Global system quadratic form (QUFETC2) = (',g24.16,1h,,g24.16,1h))

      if (rank .eq. 0) write(6,1001) soupow
      if (rank .eq. 0) write(80,1001) soupow
 1001 format('Total input power (work of sources on solution, QUFETC2) = ('
     ;,g24.16,1h,,g24.16,1h))

	if(.not.recalc_by_species_now)then
      if (rank .eq. 0) write(80,*)'i x*.A.x'
c      write(80,*)'x*.A.x'
      if (rank .eq. 0) write(80,*)nele + nreg, 5
      if (rank .eq. 0) write(82,*)'i x*.(b-A.x)'
c      write(82,*)'x*.(b-A.x)'
      if (rank .eq. 0) write(82,*)nele + nreg, 3
        ib = 0
          do ireg = 1, nreg
...
c         The factor i ensures active+i*reactive:
          if (rank .eq. 0) write(80,*)float(ib), -dimag(xsax(ib)), dreal(xsax(ib))
...
c         Weak form residual i transconj(x).(b-A.x) for each block:
          if (rank .eq. 0) write(82,*)float(ib), -dimag(xsbmax(ib)), dreal(xsbmax(ib))
...
c         Combine 2 boundary blocks, except for wall (which only has 1):
          ...
          if (rank .eq. 0) write(80,*)float(ib), -dimag(zed), dreal(zed), -dimag(zb), dreal(zb)
          if (rank .eq. 0) write(82,*)float(ib), -dimag(zed2), dreal(zed2)
...
c          write(82,*)float(ib), dreal(z2), dimag(z2)
      ...
      if (rank .eq. 0) write(80,*)'i x*.A.x'
      if (rank .eq. 0) write(80,*)nspec
	  do isp = 1, nspec
	  if (rank .eq. 0) write(80,*)paname(isp)
	  end do
	  if (rank .eq. 0) then
...
c         The factor i ensures format active+i*reactive:
          if (rank .eq. 0) write(80,*)float(iel), rnorm*(fx0(iel-1)+0.5*fl(iel)), eltvol(iel)
...
c      write(6,*)'Total input power (work of sources on solution) = ', soupow
      ...
      if (rank .eq. 0) then
        ...
        do ia = 1, nrhs
        if (rank .eq. 0) write(6,1002)(rawzij(ia,ja),ja=1,nrhs)
        end do
...
c     Antenna toroidal location and shape factors to produce actual impedance matrix:
     ...
     ;  zij_quf(ia,ja) = zij_quf(ia,ja) * cbli
        ...
        do ia = 1, ntoant
        if (rank .eq. 0) write(6,1002)(zij_quf(ia,ja),ja=1,ntoant)
        end do
...
c	write impedance matrix real and imaginary parts for easy further manipulation:
	   if (rank .eq. 0) write(80,*) ntoant, 2*ntoant
        do ia = 1, ntoant
        if (rank .eq. 0) write(80,1003)(dreal(zij_quf(ia,ja)), dimag(zij_quf(ia,ja)),ja=1,ntoant)
 1003 format(8(1h ,g23.15))
	...
	close(unit=80, status='KEEP')
     ...
     ;                                  * dconjg(tofacv(ia)) * tofacv(ja)
      ...
      if (rank .eq. 0) write(6,*)'Total input power (from raw impedance matrix, QUFETC2) = '
     ;        , matpow_raw
      if (rank .eq. 0) write(6,*)' '
     ...
     ;                          * dconjg(tancur(ia)) * tancur(ja)
      ...
      if (rank .eq. 0) write(6,*)'Total input power (from array impedance matrix, QUFETC2) = '
     ;        , matpow
      if (rank .eq. 0) write(6,*)' '
