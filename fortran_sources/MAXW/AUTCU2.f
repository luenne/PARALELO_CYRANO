...
c      include 'paralelo.copy'
     ...
     ; KILOMA,KIPRE(MAXPOM),KPILO
      ...
      IF (rank .eq. 0) WRITE(NOFILE,*)'ENTER AUTCUT ; time=',SECOND()-TIMIN
...
C     ========
  ...
  16  CONTINUE
      ...
      IF(LEL.LT.LELMIN .AND. rank .EQ. 0) 
...
C     END IF
      ...
      END IF
      IELELO = IELELO + 1
      IF (rank .EQ. 0) WRITE(6,*)IELELO,': Re on ',I1-1+MMINL,',Im on ',I2-1+MMINL,' LEL=
     ; ',LEL
      CUT(IELELO) = Y
...
C     their value at antenna:
      IF(KIMOLO)THEN
      	...
      	IF (rank .EQ. 0) WRITE(NOFILE,*)'Mode ',MMAXL,' dropped from local element ',
     ...
     ;	.AND. TDAM(MMINL+1-MMININ).GE.TDAMMA )THEN
      	IF (rank .EQ. 0) WRITE(NOFILE,*)'Mode ',MMINL,' dropped from local element ',
     ;	IELELO,' at rho=',Y*RNORM
      	MMINL = MMINL + 1
      	MMAXLR = MMAXLR - 1
      	END IF
...
C     CUT REMAINING REGIONS:
   ...
   3  CONTINUE
      ...
      IF (rank .EQ. 0) WRITE(6,*)'NELE=',NELE
...
C     STORE THE CUT IN GLOBAL VECTOR FX0:
   ...
   5  	CONTINUE
      ...
      IF (rank .EQ. 0) WRITE(NOFILE,*)(FX0(IELG),IELG=IED,KT+IED)
   4  CONTINUE
      ...
      IF (rank .EQ. 0) WRITE(NOFILE,*)'EXIT AUTCUT ; time=',SECOND()-TIMIN
      RETURN
