...
cERN   For E- component output
       real*8 myyout(nabplo)

c**********************************************************************
 
      if (rank .eq. 0) write(nofile,*)'Enter OUTRFF'
...
c     ======
      ...
      iel = i1st(isubr) + iele(isubr)
        ...
        nwr = ceilq(5,4)
          do i = 1, nwr
          if (rank .eq. 0) write(nofile,182)(j+i-1,xrtpb(j+i-1,ireg,im,1,1),j=1,5+1-i,nwr)
          end do
        end do
      end do
      end if
      ...
      if(nscree .gt. 0) then
      if (rank .eq. 0) write(nofile,*)'Number of Faraday shields =',NSCREE
        if(monomo)then
        if (rank .eq. 0) write(nofile,*)'Screen current is modulated by mode M=',moscr(1)
        else
        if (rank .eq. 0) write(nofile,*)
     ;     'Screen current is modulated by modes M=', moscr(1), ' to m=', moscr(nmoscr)
        end if
...
c     This modifies XRTPB and solution vector:
      ...
      if (rank .eq. 0) write(nofile,*)'No Faraday shield.'
      end if
...
c     AND TOTAL IMPEDANCE ZIMPTO in 3D runs
     ...
     ; // ' char.impedance ; electrical length'
        ...
        do i = 1, ntoant
        if(falen(i) .and. rank .eq. 0)write(nofile,*)i, zap(i), zcapri(i), betal(i)
        end do
      if (rank .eq. 0) then 
...
c     Compute total input power: for shorted antenna, this uses the short-circuit current.
     ...
     ;              ntoant, tancur, 1, ntoant, ctra2)
      ...
      if (rank .eq. 0) write(nofile,*)'Total input complex power=', TINPCP, ' VA'
      tinpow = dreal(tinpcp)
      losles = dabs(tinpow) .le. 1.d-10 * dabs(dimag(tinpcp))
        if(losles)then
        if (rank .eq. 0) write(nofile,*)'No significant input active power for this run'
        else
        if(.not.glovac .and. tinpow.lt.0.d0) then
		    if (rank .eq. 0) write(nofile,*)'caution: plasma is feeding antennas'
        end if
        end if
...
c     zero in 'HEC' subregions:  
      ...
      do ic = 1, 5
        do im = 1, nmode(2)
        if (rank .eq. 0) write(nofile,*)'ic=', ic, '; im=', im
          do i = 1, ist11
          if (rank .eq. 0) write(nofile,2000)
...
cERN New: Testing
c       E- component:
        myyout(i) = myyout(i) + dreal(epmloc(3,1))
        myyout(i) = myyout(i) + dimag(epmloc(3,1))
...
c	facern = 1.d0
c	if(cokpco)facern=0.92d0
c	yout = facern * yout
c	youth = facern * youth
...
cERN   Testing
	       open (UNIT = 708, FILE = paplda // 'Eminus_1D.dat', STATUS = "REPLACE", ACTION = "WRITE")
 	       write(708,*),"R.H. electric field"
	       ...
	       do j = 1, nabsci
	           ...
	           write(708,GGs), eqt(aux,1,1) , myyout(aux), myyout(aux)
...
cERN	04Feb2005: Write E_theta and H_phi poloidal modes

	GGs2 = "(g16.6, 9g16.6)"

c     1) Real E_theta
      if (rank .eq. 0) then
	     open (UNIT = 701, FILE = paplda // 'Etheta_modes_real.dat', STATUS = "REPLACE", ACTION = "WRITE")
	     write(701,*),"Poloidal electric field modes (real)"
	     write(701,"(i5,i5)"), ist11, ipla
	     write(701,GGs2), nmodc !!!, (/1:nmodc/)-1+minfc
	     do j = 1, ist11
		     aux = j
		     write(701,GGs2), eqt(aux,1,1) , dreal(xrtp(2,aux,1:nmodc))
	     end do
	     close (701)

c     2) Imag E_theta
	open (UNIT = 701, FILE = paplda // 'Etheta_modes_imag.dat', STATUS = "REPLACE", ACTION = "WRITE")
	write(701,*),"Poloidal electric field modes (imag)"
		write(701,"(i5,i5)"), ist11, ipla
		write(701,GGs2), nmodc !!! , (/1:nmodc/)-1+minfc
		do j = 1, ist11
		aux = j
			write(701,GGs2), eqt(aux,1,1) , dimag(xrtp(2,aux,1:nmodc))
		end do
	close (701)

c     3) Real H_phi
	open (UNIT = 701, FILE = paplda // 'Hphi_modes_real.dat', STATUS = "REPLACE", ACTION = "WRITE")
	write(701,*),"Toroidal magnetic field modes (real)"
		write(701,"(i5,i5)"), ist11, ipla
		write(701,GGs2), nmodc !!! , (/1:nmodc/)-1+minfc
		do j = 1, ist11
		aux = j
			write(701,GGs2), eqt(aux,1,1) , dreal(hrtp(3,aux,1:nmodc))
		end do
	close (701)

c     4) Imag H_phi
	open (UNIT = 701, FILE = paplda // 'Hphi_modes_imag.dat', STATUS = "REPLACE", ACTION = "WRITE")
	write(701,*),"Toroidal magnetic field modes (imag)"
		write(701,"(i5,i5)"), ist11, ipla
		write(701,GGs2), nmodc !!! , (/1:nmodc/)-1+minfc
		do j = 1, ist11
		aux = j
			write(701,GGs2), eqt(aux,1,1) , dimag(hrtp(3,aux,1:nmodc))
		end do
	close (701)
      end if !(rank .eq. 0) 
...
cERN  16/01/2006: Provisory power norm. factor based on Poynting Flux
c                       (discard last element)
cNEW    20/09/06: Normalize according to rfpow (input parameter)
        if(ireg.eq.1)then
          TOTALPOWER_poynt = yout(i_rp-5,trncp1)
c        POWERFAC = rfpow / TOTALPOWER_poynt
          POWERFAC=1.d0
c            print*, 'POWERFAC =', POWERFAC
            if (rank .eq. 0)print*, 'Total RF power =', POWERFAC * TOTALPOWER_poynt
c   write(nofile,*)'(OUTRFF) Applying norm. factor to 1D fields'
c   write(nofile,*)'Total power set to rfpow=',rfpow
c   write(nofile,*)'Power factor (rfpow/Poynt(ap))=',POWERFAC
         end if
...
c         i points on right node of current element, i-j2 on left node:
     ...
     ;                         yout(i,trncp2) - yout(i-j2,trncp2))
          if (rank .eq. 0)write(80,*)iel, ibl, rnorm*(fx0(iel-1)+0.5*fl(iel)), dreal(poydie(ibl)), dimag(poydie(ibl))
          if (rank .eq. 0)write(82,*)rnorm*fx0(iel), yout(i,trncp1), yout(i,trncp2)
          ibl = ibl + 1
          end do
c       This is the difference across region boundary:
     ...
     ;                       yout(i+1,trncp2) - yout(i,trncp2))
        if (rank .eq. 0) write(80,*)iel, ibl, rnorm*(fx0(iel-1)+0.5*fl(iel)), dreal(poydie(ibl)), dimag(poydie(ibl))
        if (rank .eq. 0) write(82,*)rnorm*fx0(iel), yout(i+1,trncp1), yout(i+1,trncp2)
	  end if
        end do        
...
c      end do
      if (rank .eq. 0)write(75,*)ist11, 3*ncomp+3+9+1
c2004      write(75,*)ist11, 3*ncomp+3+9
      do i = 1, ist11
c2004      write(75,2000)absr(i), (yout(i,j),j=1,3*ncomp+2), (youth(i,j),j=1,9)
      if (rank .eq. 0) write(75,2000)absr(i), (yout(i,j),j=1,3*ncomp+2), (youth(i,j),j=1,9), abscis(i)
      end do
...
c      if(.not.cyl)fac4 = fac4 * r0i
      ...
      ist = 1
        do ireg = 1, nreg
        i = ist
        dpoydr(i) = (yout(i+1,trncp1) - yout(i,trncp1)) / (absout(i+1) * 
     &              (absout(i+1) - absout(i)))
...
c  now    dFlux/(rho*drho): compare to POWDEN in OUTPOW (new scaling)
          dpoydr(i) = (yout(i+1,trncp1) - yout(i-1,trncp1)) / 
     &                (absout(i) * (absout(i+1) - absout(i-1)))
          end do
C       Set IST to 1st point of next region:
        ist = ist + istp(ireg)
        i = ist - 1
        dpoydr(i) = (yout(i,trncp1) - yout(i-1,trncp1)) / 
     &              (absout(i) * (absout(i) - absout(i-1)))
c       Jumps of complex inward Poynting flux at region boundaries:
        poyjum(ireg) = dcmplx(yout(i,trncp1), yout(i,trncp2))
        if(ireg.lt.nreg)poyjum(ireg) = poyjum(ireg) - 
     &     dcmplx(yout(ist,trncp1), yout(ist,trncp2))
        end do
...
c     ========
      ...
      if (rank .eq. 0) then
        ...
        do i = 0, nreg
        if (rank .eq. 0) write(nofile,*) i, poyjum(i)
        end do
      if (rank .eq. 0) then 
        ...
        if (rank .eq. 0) WRITE(NOFILE,3000) IM-1+MINF(IMAX), MONORM(IM)
 3000   format(1h , i4, 2x, g13.5)
        if (rank .eq. 0) WRITE(75,*) float(IM-1+MINF(IMAX)), MONORM(IM)
          DO I = 1, IST11
