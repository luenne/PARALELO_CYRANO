...
cERN  Derivatives d/dr(sin THETA) and d/dr(cos THETA) added 
c     in cvc(*,8) and cvc(*,9)
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
...
c         Curl is in rho, theta, phi components
      ...
      syv(4) = 1
      syv(5) = 1
cERN	15/03/05 : Include THETA radial derivatives
        if(rtp)then
        ncoexp = 7  + 2	! cERN
        syv(6) = 1
        syv(7) = 1
...
cERN	15/03/05 : Include THETA radial derivatives
	    dsidr = eqt(i,ipo,16)
		dcodr = - eqt(i,ipo,13) * dsidr	! (check minus sign with Ph.)
...
c         multiplication by facju to do):
          ...
          cr(ipo,6) = t * co	! cos(THETA)
          cr(ipo,7) = t * si	! sin(THETA)

cERN	15/03/05 : Include THETA radial derivatives
	    cr(ipo,8) = t * dcodr  ! d/dr[cos(THETA)]
          cr(ipo,9) = t * dsidr	! d/dr[sin(THETA)]
...
c             Curl written in R, Y, phi components
            if (rank .eq. 0) print *, 'FOUCOJ: CAR element type to write'
	      stop
...
c           ============================
c             Curl written in R, Y, phi components
            if (rank .eq. 0) print *, 'FOUCOJ: const. k// coordinates to write for CAR element type'
	      stop
            end if
c           ======
...
cERN	15/03/05 : Include THETA radial derivatives
	    ccr(ipo,8) = t * dcodr  ! d/dr[cos(THETA)]
          ccr(ipo,9) = t * dsidr	! d/dr[sin(THETA)]
