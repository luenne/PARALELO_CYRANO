...
cERN	NEW: Store partial power contributions
	real*8 :: aux_ic10, aux_lan, aux_ic2, aux_ic11, aux_ic12,
     ;          aux_el2, aux_ttmp, aux_ttmp2, sum_ic10, sum_lan,
     ;          sum_ic2, sum_ic11, sum_ic12, sum_el2, sum_ttmp,
     ;          sum_ttmp2
      complex*16
     ;  zdotc, zdotu
     ;, tra1
...
cccccccccccccccccccccc LUENNE ccccccccccccccccccccccccc
      real :: tempo_ini, tempo_fim, tempo_total		  
...
cERN	RESET partial power contributions
	...
	aux_ttmp2 = 0
	  ...
	  tempo_ini = second()
      do ipol = ((nploth/processo)*rank) + 1, (nploth/processo)*(rank+1) 
...
cERN	Store partial power contributions (poloidal average)
	...
	aux_ttmp2 = aux_ttmp2 + pttmp2
      ...
      call MPI_REDUCE (aux_ic10,sum_ic10,1,MPI_REAL, MPI_SUM, 0, 
     &                comm, error)
      call MPI_REDUCE (aux_lan,sum_lan,1,MPI_REAL, MPI_SUM, 0, 
     &                comm, error)
      call MPI_REDUCE (aux_ic2,sum_ic2,1,MPI_REAL, MPI_SUM, 0, 
     &                comm, error)
      call MPI_REDUCE (aux_ic11,sum_ic11,1,MPI_REAL, MPI_SUM, 0,
     &                comm, error)
      call MPI_REDUCE (aux_ic12,sum_ic12,1,MPI_REAL, MPI_SUM, 0, 
     &                comm, error)
      call MPI_REDUCE (aux_el2,sum_el2,1,MPI_REAL, MPI_SUM, 0,
     &                comm, error)
      call MPI_REDUCE (aux_ttmp,sum_ttmp,1,MPI_REAL, MPI_SUM, 0,
     &                comm, error)
      call MPI_REDUCE (aux_ttmp2,sum_ttmp2,1,MPI_REAL, MPI_SUM, 0,
     &                comm, error)
...
cERN	Write individual power contributions to file
      if (rank .eq. 0) then
		...
		write(109,"(9E16.8)"), abscis(i),aux_ic10, aux_lan, aux_ic2, aux_ic11,
     ;          aux_ic12, aux_el2, aux_ttmp, aux_ttmp2 
      end if !(rank .eq. 0)
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      tempo_fim = second()
	  tempo_total = tempo_total + (tempo_fim - tempo_ini)
	  conta = conta + 1
...
c     Cumulative power at element nodes, written with radial abscissa:
	...
	write(nofile,*)
	 ...
	 print *, 'Tempo Final Laço Outpow = ', tempo_total/conta
	 print *, 'Vezes = ', conta
      end if !(rank .eq. 0)
      return
