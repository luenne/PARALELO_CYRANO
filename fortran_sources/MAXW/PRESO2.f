...
c     Number of blocks for Thomas (= block tridiagonal) algorithm:
      ...
      nuca = nlca
      if (rank .eq. 0) write(nofile,101) imax, nmode(imax)
      if (rank .eq. 0) write(nofile,*) 'There are approx. ',nonec,
     ;' complex numbers in finite element blocks.'
      totvec = nk1(nele) + nmode(nele)
      totbub = nkm(nele) + nmode(nele)
      if (rank .eq. 0) write(nofile,*)totvec,' vector blocks; vector length<= '
     ;, ndof*totvec
      if (rank .eq. 0) write(nofile,*)'Number of unknowns =', ngelim
      if (rank .eq. 0) write(nofile,*)'Number of Thomas blocks =', nthoma
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc 
cERN	Write poloidal mode couplings to file 'minf_msup.dat'
	if(FLAG .and. rank .eq. 0) then
