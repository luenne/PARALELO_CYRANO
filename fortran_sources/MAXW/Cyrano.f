...
ccccccccccccccccccccccccccccccccc LUENNE ccccccccccccccccccccccccccccccccc
...
ccccccccccccccccccccccccccccccccc LUENNE ccccccccccccccccccccccccccccccccc	
      ...
      call MPI_COMM_SIZE (comm, processo, error)
      if (rank .eq. 0) print *, 'Quant processos =  ', processo
ccccccccccccccccccccccccccccccccc LUENNE ccccccccccccccccccccccccccccccccc
...
cccccccccccccc Testing M12 matrices for standard coordinates ccccccc
	WRITE_M12STD=.true.
	if(WRITE_M12STD .and. rank .eq. 0)then
...
c Not in use:     One-dimensional plots are stored in following file:
      no1dpl=10
      if (rank .eq. 0) write(nofile,*) 'enter main '
...
c     Run title and data input:
      ...
      if (rank .eq. 0) write(603,*)'reading namout ; nifile= ',nifile
      read(nifile,namout,end=1001,err=1002)
      if(wridat .and. rank .eq. 0)write(nofile,namout)
c
...
c
      ...
      if(wridat .and. rank .eq. 0)write(nofile,namplo)
      nifil2=nifile
...
c 
      if (rank .eq. 0) write(603,*)'Reading namelist data on file ',nifil2
      read(nifil2,namgeo,end=1001,err=1002)
      if (rank .eq. 0) write(nofile,*)'namgeo ok'
      read(nifil2,nammag,end=1001,err=1002)
      if (rank .eq. 0) write(nofile,*)'nammag ok'
      read(nifil2,nampla,end=1001,err=1002)
      if (rank .eq. 0) write(nofile,*)'nampla ok'
      read(nifil2,namant,end=1001,err=1002)
      if (rank .eq. 0) write(nofile,*)'namant ok'
      read(nifil2,namreg,end=1001,err=1002)
      if (rank .eq. 0) write(nofile,*)'namreg ok'
      read(nifil2,namsub,end=1001,err=1002)
      if (rank .eq. 0) write(nofile,*)'namsub ok'
      read(nifil2,nampom,end=1001,err=1002)
      if (rank .eq. 0) write(nofile,*)'nampom ok'
      read(nifil2,namsys,end=1001,err=1002)
      if (rank .eq. 0) write(nofile,*)'namsys ok'
...
c     build the abcissa vectors 'abscis' and 'abscno' in Cyrano (TABLES.f).
     ...
     ;              '----> Importing experimental data (READ_GENERAL = T)'
	        if (rank .eq. 0) write(nofile,*) 
	        if (rank .eq. 0) print *, '--> Importing general data.'
	        if (rank .eq. 0) print *
...
c     plot parameters:
      if (rank .eq. 0) write(nofile,*)'Repeat the output with different'
...
cPL10/4/2004: Create folder \runxxx\ for current run index (3-digit xxx padded with zeros if required) in Plot_data folder:
	if (rank .eq. 0) resp = SYSTEM("mkdir " // paplda)
...
C         Check elongation and Shafranov shift are admissible:
          if(kappa.le.0.)then
          if (rank .eq. 0) write(nofile,*)'Elongation KAPPA must be positive; run aborted'
          stop
          end if
          if(shsh0 .ge. 0.5*rhocur)then
          if (rank .eq. 0) write(nofile,*)'Shafranov shift larger than half 
     ;current minor radius; singular geometry; run aborted'
          if (rank .eq. 0) print *, 'STOP at Cyrano.f line 516!'
          stop
          end if
        end if
...
c       glo0or: remains true when all plasma regions are 0 order flr.
     ...
     ;                          .not.flrops(i) )
      ...
      if (rank .eq. 0) write(nofile,*)'Normalisation length rnorm =', rnorm

      if (rank .eq. 0) write(nofile,*)'Neutrality check, for each region (0=OK):' 
        do i = 1, nreg
        if (rank .eq. 0) write(nofile,*)i, chneu(i)
        end do
...
c       Antenna end type: (short or open circuit)
        ...
        if (rank .eq. 0) write(nofile,*)'Antenna #', i, ': unknown type of end connection'
		stop
          end if
c       Antenna type :
...
c	  ADDSCR (Faraday shields): to be updated
          if(samant)then
          if(ntoant.gt.1 .and. rank .eq. 0)write(nofile,*)
...
c     Max.order of coupled neighbour poloidal modes:
        ...
        if (rank .eq. 0) write(nofile,*)'Warning: ncrot=',ncrot,' > npfft/2=',npfft/2
        if (rank .eq. 0) print*, 'Warning: ncrot=',ncrot,' > npfft/2=',npfft/2
        if (rank .eq. 0) print*, 'Stop at CYRANO line 874!'
        end if
...
c     Label current run in log file :
      nofil2 = nofile
      if (rank .eq. 0) write(nofil2,*)title
        if(wridat .and. rank .eq. 0)then
        write(nofil2,namout)
        write(nofil2,namplo)
        write(nofil2,namgeo)
        write(nofil2,nammag)
      ...
      if (rank .eq. 0) write(nofil2,9000)(betaed(i),i=1,nreg-1)
      if (rank .eq. 0) write(nofil2,9001)(pedge(i),i=1,nreg-1)
...
c		Assumes mode range and mode step give exact integer division:
		ntotor = (motov2 - motov1) / motovs
	      ...
	      if (rank .eq. 0) write(nofile,*)'Max. allowed number of toroidal modes exceeded'
	      if (rank .eq. 0) write(nofile,*)'NTOTOR=', ntotor, ',but MAXTOM=', maxtom, ';I stop.'
	      stop
	      end if
...
c     -------------------------
      ...
      if(mstust.eq.0 .or. iabs(mstust).ge.1.d10)mstust=1
        ...
        if(autome)then
        if (rank .eq. 0) write(nofile,*)'Automatic mesh generation currently not working and '
        if (rank .eq. 0) write(nofile,*)'has been disabled; input mesh is used!'
        autome = .false.
        end if
...
c         Mesh was previously generated
          read(nifil2,nammes)
          if (rank .eq. 0) write(nofile,nammes)
          else 
          mst1m = mstud1
          mst2m = mstud2
          mstsm = mstust
C         This routine to revise for each new application!
        ...
        if(keepso .and. rank .eq. 0)write(nofil2,nammes)
...
c	    Use Lagrange quadratic on Erho and Hermite cubic basis functions on Etheta, Ephi
          ...
          if (rank .eq. 0) print *, 'New element type CAR under construction'
	    stop
c	    Use Hermite cubic basis functions on ER, EY, Ephi electric field components
        ...
        if (rank .eq. 0) write(nofile,2001) isubr, kt, styp(isubr)
        end do
...
cPL9/4/2004: Erase folder Plot_data and all contents if first run of this data file:
	  if(run_index .eq. 1 .and. rank .eq. 0)then
...
c       call rcfft2(1, -1, npfft, rdum, workm, vdum)
	call dfftri(npfft, work2) ! -- IMSL (real FFT) --	
       if (rank .eq. 0) write(603,*)'After init fft: npfft=', npfft
...
c	pollength = (surface perimeter) / (number pol. modes)
        if(.not.giplas .and. dynmod)then
           if (rank .eq. 0) write(nofile,*)'dynmod=T not yet compatible with giplas=F. I stop.'
           if (rank .eq. 0) print *, 'STOP at Cyrano line 1210!'
           stop
	end if
...
! ---------- Generate M12 tables for k// = const. coordinates ---------------- !
	...
	if (cokpco .and. WRITE_TABLES) then	
           if (rank .eq. 0) print*,'(Cyrano) COKPCO not reqdy!'
...
c                 for antenna spectrum (restore value at the end)
          ...
          tempo0 = second()-timin
	      if (rank .eq. 0) print *,'Enter FILANT' 
          if (rank .eq. 0) write(605,*), '--> (CYRANO.f) Begin FILANT:', tempo0, 'sec'
	  ...
	  if(STDVAC)cokpco=.false.
          ...
          tempo = second()-timin
          if (rank .eq. 0) print *, 'Exit  FILANT' 
c	  print *, '     Time used:', tempo-tempo0, 'sec'
        if (rank .eq. 0) then
	        print *
...
c       Case of plot a previously stored run:
        if (rank .eq. 0) write(603,*)'old solution to be read in file ',nifil2
c       read(nifil2,1003)
...
c     Jumps across antennae:
      if (rank .eq. 0) print *,'Enter ANJUMP'
      call anjump
      if (rank .eq. 0) print *,'Exit  ANJUMP'
      if (rank .eq. 0) print *
...
c       Generate and factorize global matrix and right-hand side; solve system:
        gensys = .true.      
        if (rank .eq. 0) print *,'--> chamada 1 do GENERA3'
        call genera3

	tempo  = second()-timin
        if (rank .eq. 0) print *
        if (rank .eq. 0) print *, '--> Exit GENERA3 (gensys = T)'
c	print *, '     Time used:', tempo-tempo0, 'sec'
        if (rank .eq. 0) then
		     print *
...
c        do i = 1, nwr
          do i = 1, lbl2
          i2 = i2 + 1
          if (rank .eq. 0) write(nofile,100)i2, bel(i,1)
...
cERN	call outdis   ! OUTPUT FILES also commented in OPENPL.COPY
	     if (rank .eq. 0) print *,'Enter OUTDIS'
	call outdis_ern
	    if (rank .eq. 0) print *,'Exit OUTDIS'
        if (rank .eq. 0) print *
...
cERN		Don't write results in this ASPLAS call
          WRITE_OUTPUT = .false.
		   if (rank .eq. 0) print *,'--> chamada 2 do GENERA3'
          call genera3
...
cERN		Don't write results in this ASPLAS call
          WRITE_OUTPUT = .false.
		   if (rank .eq. 0) print *,'--> chamada 3 do GENERA3'
          call genera3
...
c       RF fields:
	if(STDVAC)cokpco=.false.
	if (rank .eq. 0) print *,'Enter OUTRFF'
        call outrff
	cokpco = cokpco_orig 
        if (rank .eq. 0) then
             print *,'Exit OUTRFF'
...
c       Power:
        if(use_outpow)then
           if (rank .eq. 0) print *,'Enter OUTPOW (this may take some time ....)'
           call outpow
...
c       2D fields: Must call after outpow if TRANSP output required (cf normalization factor norfac):
        if(plot2d)then
	      if (rank .eq. 0) print *,'Enter OUTRF2'
          call outrf2
	      if (rank .eq. 0) then 
	          print *, 'Exit OUTRF2'
              print*
...
c       Misc.:
        if (rank .eq. 0) print *,'Enter OUTBIT'
        call outbit
	    if (rank .eq. 0) print *, 'Exit OUTBIT'
        if (rank .eq. 0) print *
        if (rank .eq. 0) write(605,*), '    --> after outbit:', second()-timin       

cccccccccccccccccccccccc
        if((nspgdr.ne.0 .or.my_nspgdr.ne.0 ) .and. DIRK)then
            if (rank .eq. 0) print*, 'Enter OUT_RFCOEF'
cx            call OUT_RFCOEF(ispgdr(1))
c            call OUT_RFCOEF_2(ispgdr(1))
            if (rank .eq. 0) print*, 'Exit OUT_RFCOEF'	    
            if (rank .eq. 0) write(605,*), '     --> after OUT_RFCOEF' , second()-timin  
        end if
cccccccccccccccccccccc

        tempo = second()-timin
        if (rank .eq. 0) print *, '--> End of OUTPUT routines!' 
c	print *, '     Time used:', tempo-tempo0, 'sec'
       if (rank .eq. 0) then
	        print *
...
c2004      end if
c     ~~~~~~
      if (rank .eq. 0) write (nofile,*) 'Exit time= ',second()-timin
...
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
 ...
 1001 continue
      if (rank .eq. 0) write(nofile,*)'End of namelist data'
c2004     Close solver system:
      call fbtclo('ALL')
      stop

 1002 continue
      if (rank .eq. 0) write(nofile,*)'Error in namelist data'
c2004     Close solver system:
      call fbtclo('ALL')
      stop
