...
c           (R/R0)*jphi,m:
            ...
            if (rank .eq. 0) print *, 'CAR element at antenna: RHS contribution to write!'
	      stop
            end if
...
c	      eminpbmp(ig,2) = cdeid(-n*(ckt(intab,ipolou(ja),4) - phibarac(ja)))
     ...
     ;     'FILRHS circ: propagation effects along antenna not implemented yet with cokpco, are ignored'
           end if
...
c         ~~~~
          if(cokpco .and. rank .eq. 0)write(6,*)'FILRHS: cokpco case not dealt with yet in non-circular... Expect anything!'
...
c             sin Theta at outlet:
          ...
          trac = dconjg(tra)
           ...
           if(falen(ja))then
           if (rank .eq. 0) write(nofile,*)'Propagation effects along antenna not implemented yet in general geometry, are ignored'
           end if
