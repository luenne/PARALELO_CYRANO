...
c      equivalence (toto,to), (totos,tos), (big1,big), (big1s,bigs)
      ...
      if (rank .eq. 0) write(nofile,*)'Enter GENERA3 ; time=',second()-timin
     ...
     ;  '-------------------------------------------------------------'
	    if(onlyab_now)then
	    if (rank .eq. 0) write(nofile,*)'Only recomputing active terms'
	    else
	    if (rank .eq. 0) write(nofile,*)'Recomputing both active and reactive terms'
	    end if
	    if(recalc_by_species_now)then
	if (rank .eq. 0) write(nofile,*)'Recomputing species by species (option recalc_by_species)'
	    else
	if (rank .eq. 0) write(nofile,*)'Recomputing sum over species (option recalc_total)'
	    end if
        end if
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      if (rank .eq. 0) write(nofile,7000)ireg
...
c     ========================
      if (rank .eq. 0) write(nofile,7001)isubr
...
c     ========================
      ...
      if (rank .eq. 0) write(nofile,7002)iel
...
c	  print *
	  ...
	  if (rank .eq. 0) write(605,*), '    --> (GENERA3.f) Begin ASCURL:', tempo0, 'sec' 
c	  print *, kappa
	call ascurl
	  tempo  = second()-timin
          if (rank .eq. 0) write(605,*), '    --> (GENERA3.f)   End ASCURL:', tempo, 'sec' 
	      if (rank .eq. 0) write(605,*), '         Time used:', tempo-tempo0, 'sec'
cJAC	  print *
	end if
...
cJAC	  print *
	  ...
	  tempo0 = second()-timin
	  if (rank .eq. 0) write(605,*) '    --> (GENERA3.f) Begin ASPLAS:', tempo0, 'sec' 
cERN	  call asplasnew(.false., idum)
	  call asplas(.false., idum)
	  tempo  = second()-timin
          if (rank .eq. 0) write(605,*), '    --> (GENERA3.f)   End ASPLAS:', tempo, 'sec' 
	  if (rank .eq. 0) write(605,*), '         Time used:', tempo-tempo0, 'sec'
cJAC	  print *
        end if
	end if
...
c       Build axis constraints in CBC, their conj.transposed in CBCTC:
        ...
        if (rank .eq. 0) write(nofile,*)'Axis constraints: ', lbl1, ' equations connecting max.', lbl2, ' columns'
          do i = 1, lbl1
            do j = 1, lbl2
...
cc        call dset(LBLOCK(1), 1.d0, sc1, 1)
        ...
        if(info .ne. 0 .and. rank .eq. 0)write(nofile,*)'cgefa3 returned info =', info
...
ccccccccccccccccccccccccccc ERNESTO cccccccccccccccccccccccc
  	  tempo0 = second()-timin
	  if (rank .eq. 0) write(605,*), '    --> (GENERA3.f) Begin FBSTEP:', tempo0, 'sec' 
        call fbstep(isca, scaf(1), scaf(1))
	  tempo  = second()-timin
          if (rank .eq. 0) write(605,*)'    --> (GENERA3.f)   End FBSTEP:', tempo, 'sec' 
	  if (rank .eq. 0) write(605,*), '         Time used:', tempo-tempo0, 'sec'
cJAC	  print *
...
c       ------
      ...
      ithoma = ithoma + 1
      if (rank .eq. 0) write(603,*)'                    ithoma=',ithoma
...
ccccccccccccccccccccccccccc ERNESTO cccccccccccccccccccccccc
	  ...
	  tempo0 = second()-timin
          if (rank .eq. 0) print*,'Enter REGJUM'
	      if (rank .eq. 0) write(605,*), '    --> (GENERA3.f) Begin REGJUM:', tempo0, 'sec' 
	call regjum(ireg,'L')
	  tempo  = second()-timin
          if (rank .eq. 0) print*,'Exit REGJUM'
          if (rank .eq. 0) write(605,*), '    --> (GENERA3.f)   End REGJUM:', tempo, 'sec' 
	      if (rank .eq. 0) write(605,*), '         Time used:', tempo-tempo0, 'sec'
c	  print *
	end if
cccccccccccccccccccccccccccccccccccccccccccccccccccccccc 
...
c     idcat = idcat + nmode(nele) * iconn(isubr)
      ...
      if(gensys)then
      if (rank .eq. 0) write(nofile,*)'system solved; time= ',second()-timin
      if (rank .eq. 0) write(nofile,*)'time spent in factorization routine= ',timfac
