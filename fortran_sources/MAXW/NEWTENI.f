...
500     J=J+1
      I=MMAX-1
      IF (rank .EQ. 0) WRITE(6,*) '  NMHNLK --- series are not converged at order ',
     .           I,' in kprl2'
      IF (rank .EQ. 0) WRITE(6,*) '  ****** --- relative error= ', ERR
520   CONTINUE
      RETURN
      END
C NMKSIM
...
300     CONTINUE
      IF(ICONVG.EQ.0) THEN
      IF (rank .EQ. 0) WRITE(6,*) '  NMKS --- series has not converged at order n=',
     .       NMAX
      IF (rank .EQ. 0) WRITE(6,*) '  ****** --- relative error= ', ERR
      ENDIF
C
...
C
      ...
      RETURN
C
900   IF (rank .EQ. 0) WRITE(6,*) 'NMKS --- Number of component distribution functions ',
     .           'NFUN=',NFUN,' exceeds NFUNMX=',NFUNMX
999   IF (rank .EQ. 0) WRITE(6,*) 'NMKS --- Execution is stopped'
      STOP
      END
C NMKSMA
C
