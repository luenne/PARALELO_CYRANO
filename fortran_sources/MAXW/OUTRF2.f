...
c**********************************************************************
      ...
      if(plot2d)then
c     ~~~~~~~~~~~~~~
      if (rank .eq. 0) write(603,*)'enter outrf2'
      iplom1 = nploth + 1
...
c     -------------------------------------------
      if (rank .eq. 0) write(603,*)'enter do 18'
      do 18 ihi = 1, 3 * nficom
        if(ploty(1,ihi).or.ploty(2,ihi).or.ploty(3,ihi) .or.
     &  (transp .and. (ihi.eq.7 .or. ihi.eq.8)))then
...
c         1,2,3:
          ii = ihi
          end if !(ihi.gt.nficom)
 
          if(ihi.ge.4 .and. ihi.le.6)then
          ifi = 2
          else
          ifi = 1
          end if !(ihi.ge.4 .and. ihi.le.6)
          if(ihi.le.3)then
          ico = ihi
          else
          ico = ihi - 3
...
c           1) E+
               if(ihi.eq.7)then
	            do i = 1, ipla
	               if (rank .eq. 0)write(1770,2222)(tab(j,i,1), j = 1, iplom1)
                  end do
	            if (rank .eq. 0) print *, 'Re E+ for QLFP ... OK!'
	            do i = 1, ipla
	               if (rank .eq. 0) write(1771,2222)(tab(j,i,2), j = 1, iplom1)
                  end do
	            if (rank .eq. 0) print *, 'Im E+ for QLFP ... OK!'
	         end if !(ihi.eq.7)

c           2) E-
               if(ihi.eq.8)then
	            do i = 1, ipla
	               if (rank .eq. 0) write(1772,2222)(tab(j,i,1), j = 1, iplom1)
                  end do
	            if (rank .eq. 0) print *, 'Re E- for QLFP ... OK!'
	            do i = 1, ipla
	               if (rank .eq. 0) write(1773,2222)(tab(j,i,2), j = 1, iplom1)
                  end do
	            if (rank .eq. 0) print *, 'Im E- for QLFP ... OK!'
	         end if !(ihi.eq.8)
...
c           3) E//
               if(ihi.eq.9)then
	            do i = 1, ipla
	               if (rank .eq. 0) write(1774,2222)(tab(j,i,1), j = 1, iplom1)
                  end do
	            if (rank .eq. 0) print *, 'Re E// for QLFP ... OK!'
	            do i = 1, ipla
	               if (rank .eq. 0) write(1775,2222)(tab(j,i,2), j = 1, iplom1)
                  end do
	            if (rank .eq. 0) print *, 'Im E// for QLFP ... OK!'
	         end if !(ihi.eq.9)
...
cERN	ccccc Writing 2D fields to separate files cccccccccccccc
        ...
        if (rank .eq. 0) print *, filename
			 
        if (rank .eq. 0) write(nofile,1000) iglplo, '2d ' // charto
...
c     ~~~~~~
      if (rank .eq. 0) close(1770)
      if (rank .eq. 0) close(1771)
      if (rank .eq. 0) close(1772)
      if (rank .eq. 0) close(1773)
      if (rank .eq. 0) close(1774)
      if (rank .eq. 0) close(1775)
