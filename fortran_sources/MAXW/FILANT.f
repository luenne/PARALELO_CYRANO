...
c     ;, thba, theab1, theab2, dtheab
     ...
     ;, foutra(-npft2:npfft,4), ft(maxpom), einpbac, rescha_spectrum(maxpom), rescha_truncated(maxpom)
     ;, cdeid, cdsinc
     
      external isrchfge, isrchfgt, nsum, dsinc, cdeid, cdsinc

      if (rank .eq. 0) write(nofile,*)'enter filant'
      pi2i = 0.5d0 * oopi
...
c           Single-strap input impedance: (engineers'convention)
          ...
          if(.not.circ .and. rank .eq. 0)write(nofile,*) 
     ;        'general geometry and cokpco: case FALEN=.T. remains to deal with!'
          end if
        ...
        modva1 = moant(1)
        modva2 = moant(1)
        if (rank .eq. 0) write(nofile,*)'mode m=',moant(1),' ,kphi=',kphi

        else
          nmoant = modva2 - modva1 + 1
	    if (nmoant .gt. maxpom) then
	    if (rank .eq. 0) write(nofile,*)
     ;   'FILANT: too many poloidal modes! I stop. nmoant=', nmoant, 'Recompile with MAXPOM >=', nmoant
	    end if
          do i = 1, nmoant
             moant(i) = modva1 + i - 1
          end do
         if (rank .eq. 0) write(nofile,*)'Mode set: m=',modva1,',',modva2,' kphi=',kphi
...
c     ... or for numerical approximation of Morse pulse.
      if (rank .eq. 0) write(nofile,*)'FILANT: assuming low field side antenna'
      tth1 = thea1(j)
...
cPL temporary test 4/2/05
cOFF          foutra(0:npfft,2) = r0orta(intab,1:npfft+1) * dnfi
...
cJAC	    call df2tcf(npfft, foutra(0,1), foutra(0,1), cwork2, cpy) ! -- IMSL -- exp[-im theta]
cJAC	    call df2tcf(npfft, foutra(0,2), foutra(0,2), cwork2, cpy) ! -- IMSL -- exp[-im theta]
            if (rank .eq. 0) print*,'(FILANT) COKPCO not ready'
            stop
cPL7/2/2005 Attempt: improved fft (Numerical Recipes): NOT FULLY DEBUGGED
c          call dftcor(foutra(0,1), foutra(0,1), -1)
c          call dftcor(foutra(0,2), foutra(0,2), -1)
...
cERN		Write FFT's	
       if (rank .eq. 0) then
	      open (UNIT = 777, FILE = paplda // 'Antenna_FFT_re.dat', 
     ;      STATUS = "REPLACE", ACTION = "WRITE")
	      write(777,*), 'Antenna FFT'
c		write(777,*), 'nspec', ' rad ', ' pla ' 		
c		write(777,"(i6, i5, i5)"), nspec, nabsci, i_rp
c		write(777,*), 'rho(m) ', paname(1:nspec)
          do i = -npft2 , npft2
		    write ( 777, "(2g16.8)" ), i, dreal(foutra(i,1))
	      end do
	      close(777)	

	      open (UNIT = 777, FILE = paplda // 'Antenna_FFT_im.dat', 
     ;      STATUS = "REPLACE", ACTION = "WRITE")
          write(777,*), 'Antenna FFT'
c		write(777,*), 'nspec', ' rad ', ' pla ' 		
c		write(777,"(i6, i5, i5)"), nspec, nabsci, i_rp
c		write(777,*), 'rho(m) ', paname(1:nspec)
          do i = -npft2 , npft2
		    write ( 777, "(2g16.8)" ), i, dimag(foutra(i,1))
	      end do
	      close(777)	
      end if ! (rank .eq. 0)
...
cPL temporary test 4/2/05
cOFF          foutra(-npft2:npft2,1) = czero
cOFF	    foutra(0,1) = cun
...
c           Estimate residual charge due to spectrum truncation: (not normalized yet!)
            do im = 1, nmoant
	      m = moant(im)
	      k1 = max(-npft2,-npft2-m)
	      k2 = min(npft2, npft2-m)
            rescha_spectrum(im) = dfloat(m) * foutra(m,1) + n * sum(acokfou(k1:k2)*foutra(m+k1:m+k2,1))
	      k1 = max(-npft2,moant(1)-m)
	      k2 = min(npft2, moant(nmoant)-m)
            rescha_truncated(im) = dfloat(m) * foutra(m,1) + n * sum(acokfou(k1:k2)*foutra(m+k1:m+k2,1))
	      end do

          if (rank .eq. 0) write(nofile,*)'cokpco - filant: residual charges from full fft spectrum'
	      do im = 1, nmoant
	      if (rank .eq. 0) write(nofile,*)moant(im), '  ', rescha_spectrum(im)
	      end do
          if (rank .eq. 0) write(nofile,*)'cokpco - filant: residual charges from truncated spectrum'
	      do im = 1, nmoant
	      if (rank .eq. 0) write(nofile,*)moant(im), '  ', rescha_truncated(im)
	      end do
...
c         Estimate poloidal spectrum range for exp[-in*(phibar-phi)]: (just for information)
	    ...
	    q = qtab(intab)
        if (rank .eq. 0) write(nofile,*)
     ;    'Estimated m limits of the exp[-in*(phibar-phi)] spectrum: ', -(1-keps)*n*q, ' to ', (1-keps)*n*q/keps
c  	    To complete when nq <0...
          if (rank .eq. 0) write(6,*)'Done with circ cokpco filant'
          end if	!cokpco
...
cERNcccccccccccccccccccccccccccccccccccccccccccccccc
...
cERNccccccccccccccccccccccccccccccccccccccccccccccccccccccc
...
cERNcccccccccccccccccccccccccccccccccccc

        if(cokpco .and. .not.circ)then
c 5/12/03 Interpolate Thetabar at antenna strap inlet and outlet:
cERN	  Problems with negative 'thea1' values, since tth1 = |thea1| is
c	  always positive (see line 142): 
cERN    Solution (01/03/05) : 'tth1' replaced by original 'thea1' value

cERN	  call interp1(polang, polang(npfft+1), ckt(intab,1,3), nabplo, npfft+1, tth1, theab1(j), 1, 'R')
cERN	  call interp1(polang, polang(npfft+1), ckt(intab,1,3), nabplo, npfft+1, tth2, theab2(j), 1, 'R')
        call interp1(polang, polang(npfft+1), ckt(intab,1,3), nabplo, npfft+1, thea1(j), theab1(j), 1, 'R')
	  call interp1(polang, polang(npfft+1), ckt(intab,1,3), nabplo, npfft+1, thea2(j), theab2(j), 1, 'R')
...
cERNccccccccccccccccccccccccccc
...
c           general geometry: to do!
            if(.not.circ)then
            if (rank .eq. 0) write(nofile,*)'FILANT: propagation effects along antenna not implemented in noncircular!'  
	      stop
            end if              	
...
cPL fixing vacuum instability problems, thought linked with residual charge due to truncated poloidal spectrum when cokpco=.true.:

cPL temporary test 4/2/05
cOFF          chaa(i,j) = czero
cPL attempt 3/2/05:
cOFF          chaa(i,j) = torfac * rescha_truncated(i) / (omegag*rhoant)

cPL attempt 4/2/05 to cancel the charge resulting from series truncation by providing a suitable toroidal current:
cPL first for full turn antenna:
cOFF		coktorcur = .true.
            if(coktorcur .and. n .ne. 0.d0)then
	      k1 = max(-npft2,moant(1)-m)
	      k2 = min(npft2, moant(nmoant)-m)
	        do k = k1, k2
	        rcurat(i,j) = rcurat(i,j) + foutra(m+k,1) * (afou(k) * dfloat(m+k)/n + bcokfou(k))
	        end do
	      rcurat(i,j) = rcurat(i,j) * torfac * r0/rhoant
	      end if
c            rescha_truncated(im) = dfloat(m) * foutra(m,1) + n * sum(acokfou(k1:k2)*foutra(m+k1:m+k2,1))
	    end if ! not cokpco or cokpco

cERNcccccccccccccccccccccccccccccccccccccccccccccccccccc
...
c       Axisymmetric Faraday shields:
	    ...
	    if (rank .eq. 0) write(nofile,*)'Faraday screen remains to implement in general geometry,'
	    if (rank .eq. 0) write(nofile,*)'and for const. k// coordinates. I stop.'
	    call exit
	    end if
          if(monomo)then
          nmoscr = 1
...
c     End of code for Faraday shields
      ...
      do ja = 1, neffan
        do i = 1, nmoant
        if (rank .eq. 0) write(nofile,1000)moant(i), cura(i,ja), rcura(i,ja), rcurat(i,ja), chaa(i,ja) / eps0
...
c     ; dimag(rcurat(i,ja)), dimag(chaa(i,ja) / eps0)
 ...
 1000 format(1h , i4, 4(1h(, g15.7, 1h,, g15.7, 1h), 2x))
      ...
      if (rank .eq. 0) write(nofile,*)'Exit filant'
      return
      end
