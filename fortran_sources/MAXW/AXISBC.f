...
c     ;nc = lblock(1) + lblock(2)
      ...
      if(crown)then
c     -------------
	if (rank .eq. 0) print *, 'B.C. at inner metal shell'
	if (rank .eq. 0) write(nofile,*) 'B.C. at inner metal shell'
	  if(.not.coldpl(1) .and. flrops(1))then
	  if (rank .eq. 0) print *, 'FLR ops. not implemented in inner wall boundary conditions'
	  if (rank .eq. 0) write(nofile,*) 'FLR ops. not implemented in inner wall boundary conditions'
	  end if
      i = 0
...
c       Etheta = 0; Ephi = 0:
        if (rank .eq. 0) print *, 'New CAR element type: must implement convolution for metallic condition in AXISBC'
	  stop
        end if
...
c       In circular concentric geometry, thetabar = theta, this deals with const. k// coordinates as well
	  if (rank .eq. 0) print *, 'CIRCULAR B.C. at axis'
        i = 0
c       NB: in D shape, elongation is the only parameter influencing axis b.c.

        if(coldpl(1) .or. .not.flrops(1))then
c       +++++++++++++++++++++++++++++++++++++
          if(eletyp.eq.'HEC')then
...
c           Natural conditions: even modes have zero radial derivative
        ...
        else  ! flrops conditions for circular (incl. D-shape kappa=1) (or also for 'CAR' element)
c       ++++
	  if (rank .eq. 0) print *, 'FLR ops. not implemented in axis boundary conditions'
	  if (rank .eq. 0) write(nofile,*) 'FLR ops. not implemented in axis boundary conditions'
c	  stop

          if(eletyp.eq.'HEC')then
c           Essential conditions: thesis (4.50)
            do mr = 1, nmode(0)
            m = minf(0) + mr - 1
            jd = (mr-1) * icolo
              i = i + 1
            ...
            if(nabcap)then  ! CHECK if nothing more for FLR ops !
c           Natural conditions: thesis (4.28)
            do mr = 1, nmode(0)
            m = minf(0) + mr - 1
            m2 = m / 2
...
c           Essential conditions: ER, EY = 0 for all m, Ephi = 0 for m nonzero, thesis (4.50)
            do mr = 1, nmode(0)
            m = minf(0) + mr - 1
            jd = (mr-1) * icolo
              i = i + 1
