      subroutine interp2(xtab, incx, reftab, incref, nref, xabs, tab, ntab)
      USE GLOBAL
      implicit none

      integer nref, ntab, incx, incref

      double precision xtab(1+(nref-1)*incx), reftab(1+(nref-1)*incref), xabs(ntab), tab(ntab)
...
c      include 'paralelo.copy'
      ...
      if(nref.lt.3 .and. rank .eq. 0)write(6,*)'error interp2: the reference table only has', nref, ' entries' 
      if(xabs(1).lt.xtab(1) .or. xabs(ntab).gt.xtab(1+(nref-1)*incx)) then
          if (rank .eq. 0) write(6,*)'interp2: warning: some values lie outside interpolating range'
	  end if
