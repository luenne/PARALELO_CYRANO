...
C
      ...
      newfil(3) = newfe .ne. 0
      soldoc = doc .ne. 0
      if(soldoc .and. rank .eq. 0)write(nofile,100)
      if(unit(1) .eq. unit(2))then
      if (rank .eq. 0) write(nofile,*)'Error in routine FBTINI: matrix and solution'
...
c     Fortran unit UNIT(I) already contains at least 1 factorization.
      ...
      nblk = wk1zzz(2,i)-1
          ...
          if (rank .eq. 0) write(nofile,*)'Warning: Fortran unit ',UNIT(I)
...
c
      ...
      if (rank .eq. 0) write(nofile,*)'From PROINI: no r.h.s.; factorization already'
...
C     I=3: Element matrices are already on unit UNIT(I).
          if(modfac.ne.1)then
      if (rank .eq. 0) write(nofile,*)'Error: to work with a previously stored matrix,'
      if (rank .eq. 0) write(nofile,*)'the value of parameter MODFAC must be set to 1'
      stop
          end if
          if(i.eq.1)then
      if(ipro.le.0 .or. ipro.gt.nprobl(i))then
      if (rank .eq. 0) write(nofile,*)'Error in PROINI: the old factorization # IPRO '
     ;,'must be between 1 and',NPROBL(I)
      if (rank .eq. 0) write(nofile,*)'but IPRO=',IPRO,' was given'
      ierror = 1
      end if
      iprobl(i) = ipro
      ...
      call aswa(i)
          ...
          if (rank .eq. 0) write(nofile,110)unit(i),int(wk2zzz(3,i)),modele
          ierror = 1
          end if
      nthoma = wk2zzz(1,i)
      tstore = wk2zzz(4,i)
...
c     NB: this is nrhs of comzzz2:
      nrhs = nrhss
      if(nthoma.le.0)then
      if (rank .eq. 0) write(nofile,200)nthoma
      ierror = 1
      else if(nthoma.gt.maxnbl)then
      if (rank .eq. 0) write(nofile,300)nthoma,nthoma
      ierror = 1
      end if
      if(nrhs.lt.0)then
      if (rank .eq. 0) write(nofile,400)nrhs
      ierror = 1
      else if(nrhs.gt.maxrhs)then
      if (rank .eq. 0) write(nofile,500)nrhs,nrhs
      ierror = 1
      else if(nrhs.eq.0 .and. modfac.eq.0)then
      if (rank .eq. 0) write(nofile,*)'nrhss=0 is inconsistent with parameter modfac=0'
      ierror = 1
      end if
      if(ierror.ne.0)stop
...
c      write(6,*)'calling iimin: nthoma,lblock=',nthoma,lblock
      ...
      if (rank .eq. 0) write(nofile,*)'Block #',IMIN,' was given a negative size'
      ierror = 1
      end if
      if(lblock(imax).gt.maxbll)then
      if (rank .eq. 0) write(nofile,600)imax,lblock(imax)
      ierror = 1
      end if
      if(ierror.ne.0)stop
...
C     Number of disk blocks needed to store r.h.s.:
  ...
  10  continue
     ...
     ;+ ceilq(lblock(nthoma)*nrhs*nwn, iobll)
      ...
      if (rank .eq. 0) write(nofile,*)' '
          if(nrhs.gt.0)then
      if (rank .eq. 0) write(nofile,*)'Factorization # ',iprobl(1),'; solution group # '
     ;,iprobl(2),':'
      if (rank .eq. 0) write(nofile,*)'*********************************************************'
          else
      if (rank .eq. 0) write(nofile,*)'Factorization # ',IPROBL(1)
      if (rank .eq. 0) write(nofile,*)'*************************'
          end if
      if (rank .eq. 0) write(nofile,*)' '
          if(newmat(1))then
     ...
     ;,' real numbers'
      ...
      nwr = ceilq(nthoma, 6)
        do 20 i = 1, nwr
        if (rank .eq. 0) write(nofile,800)(j+i-1,lblock(j+i-1),j=1,nthoma+1-i,nwr)
  20    continue
      if (rank .eq. 0) write(nofile,1000) tstore, iobll, iobll*1.d-6*tstore
 ...
 1000 format(1h , 'Global matrix and pivots storage requires   ', i7 
     ...
     ;, g10.3, ' Megabyte.')        
        if(nuni.eq.3)then
        if (rank .eq. 0) write(nofile,1001) storel, iobll, iobll*1.d-6*storel
...
c
      ...
      nwr = ceilq(nthoma, 4)
      do 28 i = 1, nwr
      if (rank .eq. 0) write(nofile,200)(j+i-1,1./rcond(j+i-1),j=1,nthoma+1-i,nwr)
  28  continue
...
c     Closes global matrix I/O unit:
      ...
      if (rank .eq. 0) write(nofile,*)'*****************************************'
              if(modfac.eq.1)then
                  ...
                  if (rank .eq. 0) write(nofile,*)'Fortran unit ',UNIT(1),
     ...
     ;iobll*(nblrd(1)+nblrd(2))
        ...
        end if !(char.eq.'ELM' .or. char.eq.'elm')
        end if !(char.eq.'MAT' .or. char.eq.'mat')
...
c
      ...
      if (rank .eq. 0) write(nofile,*)'Error in RDSOL: argument 1 should be between 1'
     ;,' and ',NPROBL(2)
      stop
      end if
c
...
c
      ...
      if (rank .eq. 0) write(nofile,*)'Error in RDSOL: argument 2 should be between 1'
     ;,' and ',nthoma
      stop
      end if
c
...
c
      if(ipro.le.0 .or. ipro.gt.nprobl(2))then
      if (rank .eq. 0) write(nofile,*)'Error in WTSOL: argument 1 should be between 1'
     ;,' and ',NPROBL(2)
      stop
      end if
c
...
c
      if(iblo.le.0 .or. iblo.gt.nthoma)then
      if (rank .eq. 0) write(nofile,*)'Error in WTSOL: argument 2 should be between 1'
     ;,' and ',nthoma
      stop
      end if
c
...
c
     ...
     ;'Error calling RDELT: solver not initialized to read element matrices'
      ...
      if (rank .eq. 0) write(nofile,*)'Error in RDELT: argument 1 should be between 1'
     ;,' and ',nprobl(3)
      stop
      end if
c
...
c
      ...
      if (rank .eq. 0) write(nofile,*)'Error in RDELT: argument 2 should be between 1'
     ;,' and ',nthoma-1
      stop
      end if
c
...
c       Add 6/8/98:
        ...
        if (rank .eq. 0) write(6,*)'CGECO2: row #',i,' is zero; solver stopped'
        stop
        end if
  1   continue
      end if
...
c Add 6/8/98:
      if(INFO .ne. 0 .and. rank .eq. 0)write(6,*)'CGEFA2 called by CGECO2 returned INFO =', info
c
C
...
c     In case of anomalous exit:
        if(istatu.ne.0) then
          if (rank .eq. 0) write(nofile,*)
     &            'aqopen: unit #',unit(iuni),' istatu=',istatu
        else
          if (rank .eq. 0) write(nofile,*)'No such unit id.', IUNI
        end if !(istatu.ne.0)
      end if !(iuni.ge.1 .and. iuni.le.3)
c 
      return
      end
c
...
c     Cray AQIO routine:
     ...
     ;           ,iqueue, istatu)
         if(istatu.ne.0) then
              if (rank .eq. 0) write(nofile,*)
     &            'aqread: unit #',unit(iuni),' istatu=',istatu
...
c     Cray AQIO routine:
      call aqwait(aqp(1,iuni),istatu)
         if(istatu.ne.1) then ! opposite to WIN-Compaq version
             if (rank .eq. 0) 
     &         write(603,*)'aqwait: unit #',unit(iuni),' istatu=',istatu
         else
             if (rank .eq. 0)write(603,*)'No such unit id.', IUNI
         end if !(istatu.ne.1)
      end if !(iuni.ge.1 .and. iuni.le.3)
c
      return
