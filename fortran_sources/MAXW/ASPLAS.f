...
c     ==========================================================
      ...
      if (rank .eq. 0) write(nofile,*)'nbmax, bllen, 2*nset0:', nbmax, bllen, 2*nset0
...
c       1) Case of constant k// coordinates -------------------------------------
        ...
        if(cokpco) then  
           ...
           if (rank .eq. 0) print*, '(ASPLAS) Not ready for cokpco in this version!'
...
c	  2) Non-Maxwellians ???? -------------------------------------------------
        ...
        if(nspgdr.gt.0) then ! + + + + + + + 
          if (rank .eq. 0) print*, '(ASPLAS)nspgrd not ready!'
cJAC		 if(onespe)then
...
c          Workspace to mul3bl: amount of free 6*6 blocks in totop(12,12,...):
           ...
           if(nwkbl .le. 0 .and. rank .eq. 0) write(6,*)'error asplas: nwkbl=', nwkbl
cPL25/8/04:
...
c	  2) Non-Maxwellians ???? -------------------------------------------------
        ...
        if(nspgdr.gt.0)then
           if (rank .eq. 0) print*,'(asplas) NSPGR NOT READY!'
cJACc       Species with a general dielectric response:
cJAC           if(onespe)then
cJACcPL@          to do: allow evaluation 1 species at a time:
