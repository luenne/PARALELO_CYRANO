...
C              
C   POLSYM=.T.: 
C     geometry is poloidally symmetrical; active only if CYL=.T., DSHAPE=.F. and GENEQ=.F.
C
C   DSHAPE=.T.:
...
C    3    In = I(r) / r**2            
C    4    q            
C    5    LAMBDAn, poloidal average of Ntn**2/(R*Jn)            
C    6    1/r dPSI/dr
...
c
     ...
     ;, gauss, totcun, g, circ_shear
     ;, jnor(npfft+1), jnorav, r0oravi, gfac, trata(npfft+1), rtr

      complex*16 cdeid

      external inscr, gauss, totcun, g, circ_shear, idamin, idamax, i1st, nsum, gfac, cdeid
...
c		call read_equi(Nrad_flu)  
                if (rank .eq. 0) print*, 'read_equi not ready'
	end if

cERN	08/03/05
	    if(READ_JTOR)then
c		Read current density from file
		   call read_current
		end if
...
cERN  NEW option for kappa profile (force kappa = 1 near magnetic axis) 
	if(dshape .and. kapprof)then
	   kappa = kappa_orig - (kappa_orig-1) * exp(-50*y*y)
	   kappa_tab(i) = kappa
	endif
c	---------------------------------------------------------------------------
...
cPL      Because these quantities are 1D radial profiles.		 
	   if(circ)then
C        Magnetic field angle sine, cosine and radial (y) derivative: (1D table for circular cross-section)
         call comag2
	   sitab(i) = si
	   cotab(i) = co
	   si1tab(i) = si1
...
c	      !!!Need to decide the link between geneq and READ_FLUX!!!
		...
		call comag1(2) ! Compute geom. coef. for circular equilibrium
		cn = 0.d0
		newmu = 1.d0
		jacn = 1.d0
		g12n = 0.d0
...
c	   eqta1d(i,10) = rho * sum(eqt(i,1:npp,7)) * 2*pi / dfloat(npp)
cPL29/1/05 fussing around: above line doesn't put correct weight on first and last points when updsym=.true.
           if(updsym)then
	     eqta1d(i,10) = rho * twopi * (2.d0*sum(eqt(i,2:npp-1,7)) + eqt(i,1,7) + eqt(i,npp,7)) / dfloat(npfft)
	     else
	     eqta1d(i,10) = rho * twopi * sum(eqt(i,1:npfft,7)) / dfloat(npfft)
	     end if

c	Begin constructing 1D tables
...
cPL29/1/05 fussing around: implementing correct weight on first and last points when updsym=.true.
            if(updsym)then 
               do ipo = 2, npp-1
				eqta1d(i,4) = eqta1d(i,4) + eqt(i,ipo,9)
				eqta1d(i,5) = eqta1d(i,5) + eqt(i,ipo,7) ** 2 / eqt(i,ipo,9)
cPL29/1/05
c				eqta1d(i,9) = eqta1d(i,9) + eqt(i,ipo,9)
c				eqta1d(i,8) = eqta1d(i,8) + eqt(i,ipo,9)
               end do
			 eqta1d(i,4) = (2.d0 * eqta1d(i,4) + eqt(i,1,9) + eqt(i,npp,9)) / dfloat(npfft)
			 eqta1d(i,5) = (2.d0 * eqta1d(i,5) + eqt(i,1,7)**2/eqt(i,1,9) + eqt(i,npp,7)**2/eqt(i,npp,9)) / dfloat(npfft)
cPL29/1/05	 eqta1d(i,8) = eqta1d(i,9) / dfloat(npp-1)
	      else
               do ipo = 1, npfft
				eqta1d(i,4) = eqta1d(i,4) + eqt(i,ipo,9)
				eqta1d(i,5) = eqta1d(i,5) + eqt(i,ipo,7) ** 2 / eqt(i,ipo,9)
               end do
               eqta1d(i,4) = eqta1d(i,4) / dfloat(npfft)
               eqta1d(i,5) = eqta1d(i,5) / dfloat(npfft)
	      end if
            eqta1d(i,8) = eqta1d(i,4)
...
c				For LAMBDAn:
				eqta1d(i,5) = eqta1d(i,5) + eqt(i,ipo,7) ** 2 / (eqt(i,ipo,1) * eqt(i,ipo,9))
c				For integrand of I(rho) and V'(rho):
				eqta1d(i,9) = eqta1d(i,9) + eqt(i,ipo,1) * eqt(i,ipo,9)
               end do
               eqta1d(i,4) = (2.d0 * eqta1d(i,4) + eqt(i,1,9)/eqt(i,1,1) + eqt(i,npp,9)/eqt(i,npp,1)) / dfloat(npfft)
               eqta1d(i,5) = (2.d0 * eqta1d(i,5) + eqt(i,1,7)**2/(eqt(i,1,1)*eqt(i,1,9)) 
     ;                                        + eqt(i,npp,7)**2/(eqt(i,npp,1)*eqt(i,npp,9))) / dfloat(npfft)
               eqta1d(i,9) = (2.d0 * eqta1d(i,9) + eqt(i,1,1)*eqt(i,1,9) + eqt(i,npp,1)*eqt(i,npp,9)) / dfloat(npfft)
c              eqta1d(i,4) = eqta1d(i,4) / dfloat(npp-1)
c              eqta1d(i,5) = eqta1d(i,5) / dfloat(npp-1)
c	         eqta1d(i,9) = eqta1d(i,9) / dfloat(npp-1)
	      else ! No up-down symmetrie
               do ipo = 1, npfft
				eqta1d(i,4) = eqta1d(i,4) + eqt(i,ipo,9) / eqt(i,ipo,1)
				eqta1d(i,5) = eqta1d(i,5) + eqt(i,ipo,7) ** 2 / (eqt(i,ipo,1) * eqt(i,ipo,9))
				eqta1d(i,9) = eqta1d(i,9) + eqt(i,ipo,1) * eqt(i,ipo,9)
               ...
               eqta1d(i,4) = eqta1d(i,4) / dfloat(npfft)
               eqta1d(i,5) = eqta1d(i,5) / dfloat(npfft)
               eqta1d(i,9) = eqta1d(i,9) / dfloat(npfft)
            end if !updsym = T
...
cERN	07/03/05
		if(READ_JTOR)then	! current density already read from file in read_current.f
		   !(normalize to jo=1 at axis)
			eqta1d(i,8) = eqta1d(i,8) * (jtor_tab(i)/jtor_tab(1)) * rho
	    else
			if(ireg.eq.1)then
				eqta1d(i,8) = eqta1d(i,8) * dmax1(1.d0-(rho*api)**2, 0.d0)**alpha * rho
			else
				eqta1d(i,8) = 0.d0
		    end if
          end if
...
c     NB if crown=.true. this fails to get the total current inside rho!
      eqta1d(1,3) = 0.d0
      do i = 2, nabsci
         eqta1d(i,3) = eqta1d(i-1,3) + (abscis(i) - abscis(i-1)) * 0.5d0 * (eqta1d(i-1,8) + eqta1d(i,8))
      end do
...
c     5.4) Trapeze integration for psi(rho):
      eqta1d(1,7) = 0.
      do i = 2, nabsci
         eqta1d(i,7) = eqta1d(i-1,7) + (abscis(i) - abscis(i-1)) * 0.5d0 * (eqta1d(i-1,6)*abscis(i-1) + eqta1d(i,6)*abscis(i))
      end do
...
c	================
         ...
         if(.not.cyl)fn = fn / ra
         eqta1d(i,1) = fn		 ! f/rho
         eqta1d(i,4) = eqta1d(i,4) * eqta1d(i,2) / eqta1d(i,1)
...
c		 cccccccccccccccccccccccccccccccc
		 bmin(i) = bmotab(i,imi)
		 bmax(i) = bmotab(i,ima)
		 if (rank .eq. 0) write(nofile,*)imi,ima,bmin(i),bmax(i)
		 bbar(i) = 0.5d0 * (bmax(i)+bmin(i))
		 delb(i) = 0.5d0 * (bmax(i)-bmin(i))
           if(delb(i).lt.1.d-6)then
c		 do ipo = 1, npp
	     ckt(i,1:npp,5) = polang(1:npp)
c		 end do
           else
c			2D table of mu, on full section:
			do ipo = 1, npp
			   t = (bbar(i) - bmotab(i,ipo)) / delb(i)
			   ckt(i,ipo,5) = dacos(dsign(dmin1(1.d0,dabs(t)),t)) * dfloat(isign(1, (ima-imi)*(ima-ipo)*(ipo-imi)))
			end do
			do ipo = ima+1, npp
...
c			cccccccccccccccccccccccccccccccccc
          ...
          if(imi.ne.1 .or. ima.ne.npp)then
             ...
             gloext = .true.
             if(wr)then
                if (rank .eq. 0) write(6,*)'i=',I,': bmin and bmax at theta=',polang(imi)
...
cPL30/1/2005 Added special evaluation for circular case
        if(circ)then
c       ============
...
c     11.1) dThbar/dTheta and 'H' coefficient: 
          ckt(i,1:npp,1) = 1.d0
c	    Makes hachi=sin(THETA)/rho the inverse of cursive H in paper:
	    hachi(i) = eqt(i,1,12)

c	     jnor(1:npp) = 1.d0 / eqt(i,1:npp,1)        ! Jn / R = here 1/R; 
c          jnorav = 2.d0 * sum(jnor(2:npp-1)) + jnor(1) + jnor(npp) 
c          jnorav = jnorav / dfloat(npfft) 
          r0oravi = dsqrt(1.d0 - (abscis(i)*r0i)**2) ! 1/(poloidal average of R0/R) = 1/bfou(0)
c     11.2) dPhibar/dTheta: 
c		ckt(i,1:npp,2) = qtab(i) * (1.d0 - jnor(1:npp) / jnorav)      ! q * (1 - (1/R)/<1/R>)
	    ckt(i,1:npp,2) = qtab(i) * (1.d0 - r0oravi * r0orta(i,1:npp)) ! q * (1 - (R0/R)/<R0/R>)

c     11.3) Thbar and Phibar, using analytical expressions: 
	    ckt(i,1:npp,3) = polang(1:npp)
	    rho = abscis(i)
	    rtr = dsqrt((r0-rho)/(r0+rho))
	    ckt(i,1:npp,4) = qtab(i) * (polang(1:npp) - 2.d0 * datan(rtr * dtan(0.5d0*polang(1:npp))))
c	    ckt(i,npp,4) = 0.d0

c     11.4) dThbar/drho: identically zero in circular; dPhibar/drho using analytical expression: 
          ckt(i,1:npp,6) = 0.d0
	    ckt(i,1:npp,7) = circ_shear(abscis(i)) * ckt(i,1:npp,4) 
     ;                     + (qtab(i)/r0oravi) * dsin(polang(1:npp)) / eqt(i,1:npp,1)
c     ;                  + (qtab(i)/dsqrt(1-(abscis(i)*r0i)**2)) * dsin(polang(1:npp)) / eqt(i,1:npp,1)
         end do  ! Radial loop

cERN 27Jan2005 (2nd trial)
c     11.5) 2D table of exp[+i.n.(Phibar-Phi)] in thetabar mesh (same as theta mesh in circular)
	    do i = 1, nabsci	! radial loop
c	    Compute exp[+i.n.(Phibar-Phi)]		  	
	    einphb(i,1:npp) = dcmplx(dcos(n*ckt(i,1:npp,4)), dsin(n*ckt(i,1:npp,4)))
c	or  einphb(i,1:npp) = cdeid_vec(n*ckt(i,1:npp,4))
	    end do
...
c       ====
	   do i = 1, nabsci
c     11.1) dThbar/dTheta and 'H' coefficient: 
cPL29/01/2005 correction of poloidal averages for up-down symmetric case:
         ckt(i,1:npp,1) = eqt(i,1:npp,7) / eqt(i,1:npp,12)            ! Nn / (sin(THETA)/rho)
c         hachi(i) = 0.d0
           if(updsym)then  
           hachi(i) = 2.d0 * sum(ckt(i,2:npp-1,1)) + ckt(i,1,1) + ckt(i,npp,1) 
	     else
           hachi(i) = sum(ckt(i,1:npfft,1)) 
	     end if
         hachi(i) = hachi(i) / dfloat(npfft) 
c	   Makes hachi the inverse of cursive H in paper:
	   if(hachi(i).ne.0.d0)hachi(i) = 1.d0 / hachi(i)
	   ckt(i,1:npp,1) = ckt(i,1:npp,1) * hachi(i)

	   jnor(1:npp) = eqt(i,1:npp,9) / eqt(i,1:npp,1)        ! Jn / R
c	   jnorav = 0.d0
           if(updsym)then  
           jnorav = 2.d0 * sum(jnor(2:npp-1)) + jnor(1) + jnor(npp) 
	     else
           jnorav = sum(jnor(1:npfft))
	     end if
           jnorav = jnorav / dfloat(npfft) 

c           if(i.eq.nabsci)then
c           do ipo = 1, npp
c           dphibar/dtheta: first method
c           ckt(i,ipo,2) = ckt(i,ipo,1) * eqta1d(i,4) - eqta1d(i,2)*eqt(i,ipo,9)/(eqta1d(i,1)*eqt(i,ipo,1))
...
c     11.2) dPhibar/dTheta: 
c		  do ipo = 1, npp
c		  ckt(i,ipo,2) = eqta1d(i,4) * (ckt(i,ipo,1) - jnor(ipo) / jnorav)  ! q*(dthetabar/dtheta - (Jn/R)/<Jn/R>)
c		  end do       
		ckt(i,1:npp,2) = eqta1d(i,4) * (ckt(i,1:npp,1) - jnor(1:npp) / jnorav)  ! q*(dthetabar/dtheta - (Jn/R)/<Jn/R>)
...
c     11.4) dThbar/drho and dPhibar/drho: 
         ...
         do ireg = 1, nreg
		  i = i + 1
		  t = 1.d0 / (abscis(i+1) - abscis(i))
	        ckt(i,1:npp,6) = (ckt(i+1,1:npp,3)-ckt(i,1:npp,3)) * t
		  ckt(i,1:npp,7) = (ckt(i+1,1:npp,4)-ckt(i,1:npp,4)) * t
		  do ig = 1, (ilael(ireg)-ifiel(ireg)+1)*(ngauss+1) - 1
			 i = i + 1
			 t = 1.d0 / (abscis(i+1) - abscis(i-1))
			 ckt(i,1:npp,6) = (ckt(i+1,1:npp,3)-ckt(i-1,1:npp,3)) * t
			 ckt(i,1:npp,7) = (ckt(i+1,1:npp,4)-ckt(i-1,1:npp,4)) * t
		  end do
		  i = i + 1
		  t = 1.d0 / (abscis(i) - abscis(i-1))
		  ckt(i,1:npp,6) = (ckt(i,1:npp,3)-ckt(i-1,1:npp,3)) * t
		  ckt(i,1:npp,7) = (ckt(i,1:npp,4)-ckt(i-1,1:npp,4)) * t
	   end do ! (ireg)

cERN 27Jan2005 (2nd trial)
c     11.5) 2D table of exp[+i.n.(Phibar-Phi)] in thetabar mesh
	      do i = 1, nabsci	! radial loop
c	         Interpolate (Phibar-Phi) to uniform thetabar mesh (now REAL)
cPL31/1/2005   call interp2(ckt(i,1:npp,3), nabplo, ckt(i,1:npp,4), nabplo, npp, polang, trata(1:npp), npp)
cPL31/1/2005: using FORTRAN90 row extracted from array as input to interp2, the stride between elements must be 1:
	         call interp2(ckt(i,1:npp,3), 1, ckt(i,1:npp,4), 1, npp, polang, trata(1:npp), npp)
c		   Compute exp[+i.n.(Phibar-Phi)]		  	
		   einphb(i,1:npp) = dcmplx(dcos(n*trata(1:npp)), dsin(n*trata(1:npp)))
c	or       einphb(i,1:npp) = cdeid_vec(n*trata(1:npp)) 
	      end do	! (radial loop)
            
	  end if ! (circ & non-circ)
c       ======
...
cERN	(NEW March 2007): Update densities ------------------------------
c	Used when i_special(isp).ne.0, in which case the density
c	profile is read from file and majority density is updated
c	to assure quasi-neutrality (ne fixed)
c	NB: Non-maxw. species from BATCH are usually in this category

	do isp = 1, nspec
	   if(i_special(isp).ne.0)then
	  	  call correct_densities(isp)
	   end if
	end do
c	------------------------------------------------------------------
...
cERN  The (R,Z) grid used for PLOTTING is written in OUTGRID.f
...
c	open (UNIT = 777, FILE = paplda // 'Rall.dat', STATUS = "REPLACE", ACTION = "WRITE")
c		write(777,*), 'R coordinate'
c		write(777,*), ' rad ' , ' pol ', ' pla ' 
c		write(777,"(i5, i5, i5)"), nabsci, npfft+1, i_rp 
c		do j = 1, nabsci	
c		write(777,2222)(eqt(j,ipo,1), ipo = 1, npfft+1)
c		end do
c	close(777)

c    	13.2) Z(rho,theta) coordinate
c	open (UNIT = 777, FILE = paplda // 'Zall.dat', 
c     ;      STATUS = "REPLACE", ACTION = "WRITE")
c		write(777,*), 'Z coordinate'
c		write(777,*), ' rad ' , ' pol ', ' pla ' 
c		write(777,"(i5, i5, i5)"), nabsci, npfft+1, i_rp 
c		do j = 1, nabsci	
c		   write(777,2222)(eqt(j,ipo,2), ipo = 1, npfft+1)
c		end do
c	close(777)
