...
c     must be given in NAMPLA!
      if (rank .eq. 0) write(undirk,*)nraddr
        do ir = 1, nraddr
c        i = isrchfge(ist11, absout, 1, raqlfp(ir))
c     Assumes absout and abscis identical:
        i = irafp(ir)
        if (rank .eq. 0) write(nofile,*)'writing rf fields for QLFP on unit ', undirk
        if (rank .eq. 0) write(nofile,*)'for minor radius rho= ', absout(i)
...
c       Write index of radius and radius:
        if (rank .eq. 0) write(undirk,*)ir, absout(i)
c       Pol. mode interval, assumed the same for all elements:
        if (rank .eq. 0) write(undirk,*)minf(2), msup(2)
c       Write +, -, // components of each mode:
          do im = 1, nmode(2)
          if (rank .eq. 0) write(undirk,2000)(norfac(ic) * xpmp(2*ic-1,i,im), ic = 1, 3)
          end do
        end do
...
C     Moduli of antenna current norms:
      if (rank .eq. 0) WRITE(75,*)'Antenna current and charge/Eps0 modes moduli:'
      if (rank .eq. 0) WRITE(75,*)nmoant, 3
      do im = 1, nmoant
        tra1 = czero
        tra2 = czero
          do ia = 1, ntoant
          ...
          if (rank .eq. 0) write(75,*)float(moant(im)), (ancplo(im,j), j = 1, 2)
      end do
...
c     Antenna current modes:
      ...
      iglplo = iglplo + 1
      if (rank .eq. 0) write(nofile,1000)iglplo, title
      ploleg(iglplo) = title
...
c     Wall dissipation:
        ...
        wallos = sum1 * 0.5d0 * walres / skiwal
          if(cyl)then
          if (rank .eq. 0) write(nofile,*) 'Losses in wall=', WALLOS, ' Watt/m'
          else
          if (rank .eq. 0) write(nofile,*) 'Losses in wall=', WALLOS, ' Watt'
          end if
        if(.not.glovac .and. .not.losles) then 
		     if (rank .eq. 0) write(nofile,*)'Losses in wall=', 100*WALLOS/TINPOW, '% of input power'
		end if
        end if
