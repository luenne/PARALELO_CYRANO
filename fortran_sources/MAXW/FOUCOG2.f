...
c     Singular matrix elements * y:
      ...
      ncoexp = l
	  if(wri(1).eq.0)then
          if (rank .eq. 0) write(6,*)'curl.curl: number of coefficients = ', ncoexp
	  wri(1) = 1
	  end if
      if(l.gt.ncorot .and. rank .eq. 0)
...
c     sin and cos of magnetic angle THETA, for coordinate transformations between modes:
      ...
      ncoexp = l
	  if(wri(2).eq.0)then
          if (rank .eq. 0) write(6,*)'curl: number of coefficients = ', ncoexp
	  wri(2) = 1
	  end if
      end if
c     ------
...
C           ============================
C           Curl written in R, Y, phi components
      if (rank .eq. 0) print *, 'FOUCOG2: element type CAR in constant k// coord. to write'
	stop
            end if
c           ======
...
c     Number of coefficients:
      ncoexp = l
	  if(wri(3).eq.0)then
          if (rank .eq. 0) write(603,*)'curl.curl: number of coefficients = ', ncoexp
	  wri(3) = 1
	  end if
...
c           ============================
      if (rank .eq. 0) print *, 'CAR element FOUCOG2: at work line 892'
	stop
      i = intab
      rho = abscis(i)
      do ipo = 1, npp
...
cERN	cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
...
c     Singular terms: *y/y, so do nothing:
      ts = jacn / dfloat(npfft)
cERN	Test thetabar normalization : 
c	ts = jacn / (dfloat(npfft)*ckt(i,ipo,1))
...
c     ----
      t = 1.d0 / dfloat(npfft)
cERN	Test thetabar normalization : 
c	ts = 1.0d0 / (dfloat(npfft)*ckt(i,ipo,1))
...
c           ============================
            if (rank .eq. 0) print *, 'FOUCOG2: CAR element in const. k// to write, line 1298'
		stop 
            end if
c           ======
