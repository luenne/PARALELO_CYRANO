      subroutine polfft(v, ldv, onespe, isp, gdrtoo, switch, trafo, ipath, onlyab)

cPL 3/4/2004: suppressed one argument before last (nfft), obsolete.

cERN	4/3/2005: include FLR2 option for DSHAPE geometry
c	Elements that require multiplication by cos(THETA) 
c	before FFT included here : v(:,20) ... v(:,28) = 9 elements. 
...
c            When =1: prepares index lists; when not 1, bypasses index list building.
      ...
      integer
     ;  nfc, index(28,2), ntr(2), isp1, isp2
...
c     valid for up-down asymmetric equilibria:
     ...
     ;, tem, tem3, temk, temk2, co2, si2, s7
      ...
      complex*16 sigsd(3,3,nfc), sigod(3,3,nfc), v1(28)
     ;, a, b, c
...
cERN: Problems in FUJITSU (trafol is not always trafo!!!!) ------------
cERN  This gives problems with the number of poloidal points 'npoi'
c     SOLVED: Need to use PGI switch -Msave or save variable explicitly
...
c           Number of Fourier transforms:
            ...
            ntr(1) = 0
            ntr(2) = 0
              do lres = -1, 1 ! cERN 04/03/05 : Include second harmonic heating
...
cERN	04/03/05: Include new terms needed for DSHAPE 
c			 (multiply by cos(THETA) before doing FFT)


          ntrans = 19+9	! 9 new terms needed in DSHAPE
c         The 28 coefficients are:
c@
            if(res(1))then
c           Fundamental (p=+1/-1) ion cyclotron resonance is present
			
			ntr(1) = 12+6		! Resonant elements (6 added for DSHAPE)
			  index(1,1) = 1
			  index(2,1) = 3
			  index(3,1) = 4
			  index(4,1) = 6
			  ...
			  index(13,1) = 20	! DSHAPE
			  index(14,1) = 22	! DSHAPE
			  index(15,1) = 23	! DSHAPE
			  index(16,1) = 24	! DSHAPE
			  index(17,1) = 26	! DSHAPE
			  index(18,1) = 28	! DSHAPE		
			
			ntr(2) = 7+3		! Non-resonant terms (3 added for DSHAPE)
			  index(1,2) = 2
			  index(2,2) = 5
			  index(3,2) = 12
			  index(4,2) = 13
			  ...
			  index(8,2) = 21	! DSHAPE
			  index(9,2) = 25	! DSHAPE
			  index(10,2) = 27	! DSHAPE


            else if(res(2))then
c           Second harmonic (p=+2/-2) ion cyclotron resonance is present
            
			ntr(1) = 3+2		! Resonant elements (2 added for DSHAPE)
			  index(1,1) = 4
			  index(2,1) = 14
			  index(3,1) = 17
			...
			ntr(2) = 16+7		! Non-esonant elements (7 added for DSHAPE)
			  index(1,2) = 1
			  index(2,2) = 2
			  index(3,2) = 3
			  index(4,2) = 5
			  ...
			  index(17,2) = 21	! DSHAPE
			  index(18,2) = 22	! DSHAPE
			  index(19,2) = 23	! DSHAPE
			  index(20,2) = 24	! DSHAPE
			  index(21,2) = 25	! DSHAPE
			  index(22,2) = 27	! DSHAPE
			  index(23,2) = 28	! DSHAPE

            else
c           No fundamental or second harmonic ion cyclotron resonance is present
c           All coefficients are assumed poloidally smooth
c           (Excludes electron cyclotron resonances and anomalous cyclotron damping on p<0)
            ntr(1) = 0
            ntr(2) = 19+9   ! 9 added for DSHAPE
              do i = 1, ntr(2)
              index(i,2) = i
              end do
            end if
...
c	Valid for both CIRC and DSHAPE
      call hottev(npoi, kparv, sigsd, sigod, onespe, isp, gdrtoo, onlyab)
 
c      write(603,*),'hot:',mave,sigsd(1,1,1)
      else
...
cERN	04/03/05: New terms included for FLR2 option in DSHAPE
...
c@ CHECK: tensor in r,t,p coord.for M23 element: must have proper ldif in genera
          v1(1) = sigsd(1,1,ir) * tem
c          write(603,*),'v1(ipo):', ir, sigsd(1,1,ir)
          v1(2) = sigsd(2,2,ir) * tem
          v1(3) = sigsd(3,3,ir) * tem
...
cERN	04/03/05 : Added FLR2 contributions to DSHAPE -----------------------------
            if(.not.coldpl(ireg) .and. flrops(ireg))then
...
c			FLR2 terms:
			...
			v1(20) = v1(4) *  eqt(intab,ipo,14)		! New DSHAPE term (* cos(THETA) )
			v1(21) = v1(5) *  eqt(intab,ipo,14)		! New DSHAPE term
			v1(22) = v1(9) *  eqt(intab,ipo,14)		! New DSHAPE term
			v1(23) = v1(10) * eqt(intab,ipo,14)		! New DSHAPE term
			v1(24) = v1(11) * eqt(intab,ipo,14)		! New DSHAPE term
			v1(25) = v1(13) * eqt(intab,ipo,14)		! New DSHAPE term
			v1(26) = v1(14) * eqt(intab,ipo,14)		! New DSHAPE term
			v1(27) = v1(15) * eqt(intab,ipo,14)		! New DSHAPE term
			v1(28) = v1(16) * eqt(intab,ipo,14)		! New DSHAPE term
...
c         Some FLR terms in jump conditions (same as circular ?????)
c		FLR0 terms don't contribute??
          v1(1) = - sigod(1,1,ir) * temk
          v1(2) =   sigod(1,1,ir) * tem
          v1(3) = - sigod(1,2,ir) * temk
          v1(4) =   sigod(1,2,ir) * tem
...
c       write(603,*),'v1(fft):',v1(1)
c      write(603,*)v(1,1),v(1,2),v(1,3),v(1,4),v(1,5:10)
...
ccccccccccccccccccccccccccccccccccccccccccccccccccccccc
	  ...
	  if (rank .eq. 0) write(6,*)'Error in POLFFT: nfft is neither npfft nor nftnr!'
	  if (rank .eq. 0) write(6,*)'nfft=',nfft
	  stop
	  end if
ccccccccccccccccccc ERNESTO cccccccccccccccccccccccccccc
c        write(603,*),v(1:10,1)
c        write(603,*),v(1:10,2)
c        write(603,*),v(1:10,5)
c	print *, v(1:10,1)
